...
' Program Name : board_list_search_output.asp
...
'---------------------------------------------------------------------
'SQL Query 생성
strSQL = "Select * From my_board Where " & strSearchField & " Like '%" & _
...
%>
	...
	%>
				<tr>
					...
					<td align="center"><a href="./board_view.asp?num=<%=objRS.Fields("Num")%>"><%=objRS.Fields("Title")%></a></td>
	...
	%>
						<a href="board_list_search_output.asp?intpage=<%=(intPage-1)%>&strsearchfield=<%=strSearchField%>&strsearchquery=<%=strSearchQuery%>">[이전]</a>
	<%
		Else
	%>
	...
	%>
						<a href="board_list_search_output.asp?intpage=<%=(intPage+1)%>&strsearchfield=<%=strSearchField%>&strsearchquery=<%=strSearchQuery%>">[다음]</a>
	<%
		Else
	%>
